...
def remove_outliers(
    ...
    filtered = [
        v for v in videos
        if (v.get('channel', {}).get('subscriberCount') or 0) >= min_subscribers
    ]
...
def split_top_performers(
    ...
    top_videos = [
        v for v, val in zip(videos, metric_values) if val >= threshold
    ]
...
def _align_weights(videos: list, all_weights: List[float],
                   has_analysis_fn: Callable) -> List[float]:
...
def generate_content_type_profile(content_type: str, videos: list) -> dict:
    ...
    all_thumb_w = _align_weights(
        all_videos, all_weights, lambda v: v.get('thumbnail_analysis'))
    top_thumb_w = _align_weights(
        top_videos, top_weights, lambda v: v.get('thumbnail_analysis'))

    if all_thumb:
        profile['thumbnail'] = {
            'sampleSize': {'all': len(all_thumb), 'top10': len(top_thumb)},
            'features': compute_feature_profile(
                all_thumb, top_thumb, all_thumb_w, top_thumb_w),
        }
    ...
    all_title_w = _align_weights(
        all_videos, all_weights, lambda v: v.get('title_analysis'))
    top_title_w = _align_weights(
        top_videos, top_weights, lambda v: v.get('title_analysis'))

    if all_title:
        profile['title'] = {
            'sampleSize': {'all': len(all_title), 'top10': len(top_title)},
            'features': compute_feature_profile(
                all_title, top_title, all_title_w, top_title_w),
        }

    # Description profile (extracted from title_description analysis)
    all_desc = [
        desc for v in all_videos
        if v.get('title_analysis')
        and (desc := _extract_description_analysis(v['title_analysis']))
    ]
    top_desc = [
        desc for v in top_videos
        if v.get('title_analysis')
        and (desc := _extract_description_analysis(v['title_analysis']))
    ]

    if all_desc:
        _has_desc = lambda v: v.get('title_analysis') and _extract_description_analysis(v['title_analysis'])
        ...
        profile['description'] = {
            ...
            'features': compute_feature_profile(
                all_desc, top_desc, all_desc_w, top_desc_w),
        }
    ...
    if all_thumb and len(all_thumb) >= 30:
        ...
        if all_title and len(all_title) >= 30:
            profile['featureCorrelations']['title'] = compute_feature_correlations(
                all_title, top_title)

    # Engagement profile (split by engagementRate instead of VPS)
    _, eng_top_videos, eng_threshold = split_top_performers(
        videos, metric_fn=get_engagement_rate)

    if eng_top_videos:
        eng_top_thumb = [
            v['thumbnail_analysis'] for v in eng_top_videos
            if v.get('thumbnail_analysis')
        ]
        eng_top_title = [
            v['title_analysis'] for v in eng_top_videos
            if v.get('title_analysis')
        ]
...
def main():
    ...
    parser.add_argument(
        '--channel', '-c',
        help='Single channel ID to process (dry run — only loads this channel)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Skip saving to Firestore (local files only)'
    )
    ...
    print(f"  Removed {outlier_stats['removedLowSubscribers']} videos from low-subscriber channels (<{outlier_stats['minSubscribers']} subs)")
    ...
    if args.type in ['all', 'bridge']:
        ...
        bridge_docs = generate_recommender_documents(
            generated_profiles, gap_report
        )
