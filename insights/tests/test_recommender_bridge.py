...
import pytest
from insights.src.profiler import _get_nested
...
class TestBuildThumbnailInsights:
    def test_basic_structure(self):
        profiles = {
            'recipe': _make_profile(thumb_features={
                'humanPresence': {
                    'hasFace': {
                        'all': 0.4, 'top10': 0.8,
                        'confidence': 'high', 'significant': True,
                    },
                },
                'food': {
                    'hasFood': {
    ...
    def test_worst_performing(self):
        profiles = {
            'recipe': _make_profile(thumb_features={
                'humanPresence': {
                    'hasCartoon': {
                        'all': 0.5, 'top10': 0.1,  # Low lift = worst
                        'confidence': 'high', 'significant': True,
                    },
                },
            }),
...
class TestBuildTitleInsights:
    def test_basic_structure(self):
        profiles = {
            'recipe': _make_profile(title_features={
                'structure': {
                    'pattern': {
                    ...
                    'characterCount': {
                        'all_avg': 45.0, 'top10_avg': 50.0,
                        'confidence': 'high',
                    },
                    'wordCount': {
                        'all_avg': 8.0, 'top10_avg': 9.0,
                        'confidence': 'high',
                    },
                },
            }),
    ...
    def test_optimal_length(self):
        profiles = {
            'recipe': _make_profile(title_features={
                'structure': {
                    'characterCount': {
                        'all_avg': 45.0, 'top10_avg': 55.0,
                        'confidence': 'high',
                    },
                    'wordCount': {
                        'all_avg': 8.0, 'top10_avg': 10.0,
                        'confidence': 'high',
                    },
                },
            }),
...
class TestGenerateRecommenderDocuments:
    def test_all_four_docs_present(self):
        ...
        content_gaps = {
            'contentGaps': {
                'highOpportunity': [{'topic': 'test', 'avgViewsPerSubscriber': 5.0, 'videoCount': 10, 'opportunityScore': 2.0}],
