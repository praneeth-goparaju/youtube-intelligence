...
def main():
    ...
    channels = list(
        ...
        .where(filter=FieldFilter('analyzeEnabled', '==', True))
        .stream()
    )
    ...
    for idx, ch_doc in enumerate(channels):
        ...
        if total == 0:
            results.append({
                'channel': ch_title, 'category': category,
                'total': 0, 'thumb_done': 0, 'thumb_left': 0,
                'td_done': 0, 'td_left': 0,
            })
        ...
        thumb_done = 0
        thumb_refs = [
            videos_ref.document(vid).collection('analysis').document('thumbnail')
            for vid in non_short_ids
        ]
        ...
        td_done = 0
        td_refs = [
            videos_ref.document(vid).collection('analysis').document('title_description')
            for vid in non_short_ids
        ]
        ...
        results.append({
            'channel': ch_title, 'category': category,
            'total': total, 'thumb_done': thumb_done,
            'thumb_left': total - thumb_done,
            'td_done': td_done, 'td_left': total - td_done,
        })
        print(f"  [{idx+1}/{len(channels)}] {ch_title}: {total} vids, T:{thumb_done}/{total}, TD:{td_done}/{total}", flush=True)
    ...
    for r in results:
        ...
        print(f"{r['channel']:<35} {cat:<20} {r['total']:>6} {r['thumb_done']:>6} {r['thumb_left']:>7} {r['td_done']:>6} {r['td_left']:>8}")
    ...
    for cat in sorted(cat_totals.keys()):
        t = cat_totals[cat]
        print(f"{cat:<25} {t['total']:>8} {t['thumb_done']:>11} {t['thumb_left']:>11} {t['td_done']:>9} {t['td_left']:>9}")
        for k in grand:
            grand[k] += t[k]

    print("-" * 100)
    print(f"{'GRAND TOTAL':<25} {grand['total']:>8} {grand['thumb_done']:>11} {grand['thumb_left']:>11} {grand['td_done']:>9} {grand['td_left']:>9}")
    print("=" * 100)
